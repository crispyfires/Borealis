2.2.4:

Quick recompile to fix file download error

2.2.3:

Added:
- Rally's Water Foam
- Landscape Fixes For Grass Mods
- Improved Loading Screen Colors
- Animated Forge Water
- Praedy's Night Sky AIO - SE
- HORNCANDLES
- Skyking Unique Signs
- Skyking Signs
- Window Shadows Ultimate
- A Nirnroot
- Hired Thugs Fixed SSE
- Thugs not Assassins
- Butterflies Unchained
- Hunters Not Bandits
- Green Thumb Fix
- Argonian Creation Extended
- K.C.C.E Khajiit Character Creation Extended
- Firmament - Replace Survival Perks
- CC Wild Horses - Nordic Horse Names
- Faultier's PBR Armors and Clothes WIP
- Window Shadows Ultimate
- Simplicity of Splatter - High-Res Blood Textures
- Classes Lite - Assigned Starting Skills

Removed:
- Water Effects Brightness and Reflection Fix
- aMidianBorn - PBR and Complex Material
- Paradigm - A Quick Start Overhaul
- Fires Hurt RE
- Fire Hurts NG Updated

Updated:
- powerofthree's Papyrus Extender
- SkyPatcher
- Perk Entry Point Extender
- Actor Value Generator
- Pandora Behaviour Engine Plus
- PGPatcher (aka ParallaxGen)
- Save and Load Accelerator for SKSE Cosaves - S.L.A.C.K.
- Assorted mesh fixes
- Assorted Behavior Fixes
- Embers XD
- Sconces of Solitude - Unique Solitude Braziers - Base Object Swapper
- Weapons and Armors Remastered - PBR and Complex Materials
- Azurite III CS
- Ambient Templates for Lighting Mods
- True Light
- Torch Mechanics Fixed
- Lock Bashing
- Thieves Guild Holdup (Thief Road Encounter) Improvements
- SkyParkour v3 - Procedural Parkour and Climbing Framework (SPPF)
- lilebonymace's patches and xEdit scripts
- DynDOLOD Resources SE 3
- Survival Mode Improved - SKSE

Changes:
- Fixed a bunch of landscape issues
- Hand-corrected snow and ash shader colour to match the landscape snow and ash
- Switched Extended alternate start to Optional Quick Start + Classes Lite for classes
- Fixed an error with Bethini

2.2.2:

Added:
- Azurite Weathers and Seasons
- Azurite III CS
- Azurite Weathers - MCM and Settings Loader
- Realistic Azurite Weathers - RAW
- Misc. College of Winterhold Tweaks
- Misc. College of Winterhold Tweaks - Arniel's Appeals Letters Fix

Removed:
- Vanilla CS
- No Dead Twig Branches - Tweak for Happy Little Trees (was causing missing LOD models)

Updated:
- Thieves Guild Holdup (Thief Road Encounter) Improvements
- No Grass In Objects
- Torch Mechanics Fixed
- Assorted Behavior Fixes
- Ambient Templates for Lighting Mods
- True Light

Changes:
- Better performance for LOD, regenned without HD Tree LOD

2.2.1:

Added:
- Assorted Behavior Fixes
- Dual Wield Parrying SKSE
- Dual Wield Parrying SKSE - Mouse Buttons Fix

Removed:
- Archery Locational Damage

Updated:
- Vanilla CS
- No Grass In Objects
- Call of Valor Tweaks

Changes:
- Archery Locational Damage apparently has a bad conflict with Constellations that causes CTDs with bows, so removed it in favour of Locational Damage AE's archery feature
- You can now dual wield parry, have it set to the middle mouse button by default (you can change it in Dual Wield Parrying SKSE's ini)
- Fixed broken LOD models on trees 

2.2.0:

Added:
- Remove Small Rocks
- Remove Small Ants
- No Dead Twig Branches - Tweak for Happy Little Trees
- Skyland Bits and Bobs - A Clutter Overhaul
- Imperial Castles of Skyrim - Fort Battlements v2
- Forts of the Old Empire (1K - 4K)
- Stacks of Septims - 3d Coin Piles
- Weapons and Armors Remastered - PBR and Complex Materials
- Vine Maple Redone
- ElSopa - Training Dummies Redone
- Icy Mesh Remaster - Ice Glaciers - LOD - other fixes
- Simplicity of Sea - Water Mod with ENB and Community Shaders Displacement Textures
- Simplicity of Sea - Water Color and Transparency Tweaks
- Natural Waterfalls
- CS Particle Patch
- Chicken Replacer - Mihail Monsters and Animals (SE-AE version) (''bird'')
- Optimized Meshes - Chickens and Chicks- Mihail Monsters and Animals (SE-AE version)
- Mihail's Chickens - Cleaned and Optimized Textures
- Exist's Caves - PBR Retexture
- Acoustic Space Improvement Fixes (Corrected Reverb - Ambience - Interiors - SkyPatcher)
- Audio Overhaul - Immersive Sounds Integration (AOS - ISC Compatibility Patch)
- Immersive Sounds - Compendium
- Audio Overhaul for Skyrim SE
- Reverb Interior Sounds Expansion (SRD - Rain Thunder Ambience - Fixes)
- Beating Daedra Sound Mod (Plus Animated Heart Mesh - Particle Light - CS ENB)
- Unofficial High Definition Audio Project
- Better Argonian Horns
- The Northerner Diaries - Immersive Edition (music by Jeremy Soule)
- Still - Skyrim Inspired Music
- Hun Lovaas - Skyrim Fan-Made combat music
- Melodies of Civilization - Skyrim Fan-Made Music
- Around the Fire - Skyrim Fan-Made Music
- BA Bard Songs
- Dawnguard Music Overhaul - Skyrim Fan-made Music
- Chapter II - Jeremy Soule Inspired Music
- Yet Another Music Merge
- Footprints
- SPID for Footprints
- SPID for Footprints fix
- Footprints - Alternative Design
- Realistic PBR Footprints
- ElSopa - Creep Cluster SE
- Ash Creep Cluster SE
- High Poly Trama Roots
- Rule 11 NoDelete Automation
- Wabbajack NoDelete Automatic Indexer plugin for MO2

Removed:
- SMIM Barrels Retexture
- chair vanilla style - immersion
- 4K SMIM Furniture Chest
- 4K SMIM Furniture Improvement
- Skyrim Remastered - Metal Cages and SMIM Imperial Jails
- Static Mesh Improvement Mod - SMIM (Upscaled) (from Vanilla Remastered (Patches))
- Weathered Apiary Texture - 8K - 4K - 2K
- FYX - SMIM Barrels - Smooth and New optionnal Metal and Wood Texture
- Water for ENB
- CC's HQ Caves
- CC's HQ Carts
- Improved Mesh Vanilla Handcart Replacement (broken collision)

Updated:
- Thieves Guild Holdup (Thief Road Encounter) Improvements
- Dynamic String Distributor (DSD)

Changes:
- Added new vanilla-like music to accompany the vanilla music (not replace it) for the Extended Profile
- Higher quality music and voices, courtesy of Unofficial High Definition Audio Project
- NPCs and the player character will now make footprints on sand and snow
- Packed various mods into BSAs
- Removed ugly small rocks dotting the landscape
- Removed ugly dead 2D branches from trees
- Remove ugly ants from tree stumps
- Imperial forts now look like proper forts and aren't in a sorry state of decay
- Found some good replacers for some ugly plants
- Did another DynDOLOD tweak to reduce VRAM usage
- Removed Water for ENB because even the vanilla coloured version was causing CTDs on ElderTeej and I don't want to deal with that here
- Added some MO2 plugins to make modifying the list easier

2.1.1:

Changes:
Quick recompile to fix a Synthesis version mismatch preventing the list from being installed.

2.1.0:

Added:
- Wrye Bash (I don't use it, but if anyone wants it)
- NifSkope
- Beard Mask Fix - Beards Use Slot 44 SKSE Plugin
- aMidianBorn - PBR and Complex Material
- Frankly HD - PBR Remaster
- Skyrim Cell load Freeze fix NG
- Sloppy Vanilla Landscapes - Parallax and PBR
- PBRed Vanilla Clothes - New made from scratch
- Ancient AF Windhelm - Windhelm Retextured
- Skyland Riften
- Praedy's College of Winterhold - SE
- Praedy's Castle Volkihar - SE
- Skyland - Solitude
- Auto Resolution
- Stealth Detection Fixes
- Stretched Snow Begone Definitive Edition - Stretched Snow On Walls and Buildings Fix
- Unique Falmer Blood Elixir - Base Object Swapper
- Icy Dungeons Pack
- Icy Landmarks - Base Object Swapper
- Icy Stalhrim Source - Base Object Swapper
- JS Dragon Claws AE (Anniversary Edition)
- HD Reworked Vale Sabre Cat
- HD Reworked Vale Deer
- HD Reworked Wolves
- HD Reworked Dogs
- HD Reworked Trolls
- HD Reworked Mammoths 4K
- HD Reworked Bears 4K
- HD Reworked Horses
- Highlandcow HD by Pfuscher
- Superior Chaurus - 4k Retexture
- GOAT HD by Pfuscher
- Superior Mudcrab - 4k Retexture
- Rabbit Replacer- Mihail Monsters and Animals (SE-AE version)
- Horker by Pfuscher
- SABRECAT
- MM - REAL ELKS
- HD Skeever
- HD Reworked Pelts
- Photoreal Foxes 2K HD Retexture
- Creation Club Alternative Armors - Complex Material and PBR
- Creation Club Spell Knight Armor - Complex Material and PBR
- JS Shrines of the Divines SE
- Iconic Statues
- Detailed Rugs
- Complex Silverware - PBR and Complex Material
- Unreal 4K-8K Mammoth Skeleton ReTexture
- Weathered Apiary Texture - 8K - 4K - 2K
- Temples of the Ancients (2K - 4K)

Removed:
- Ancient Nord Armors and Weapons Retexture SE
- Bonemold Armors and Weapons Retexture SE
- Chitin Armors Retexture SE
- Common Clothing Retexture SE
- Daedric Armors and Weapons Retexture SE
- Dark Brotherhood Armors Retexture SE
- Dawnguard Armors and Weapons Retexture SE
- Divine Crusader Retexture SE
- Dragon Armors and Weapons Retexture SE
- Dragon Priests Retexture SE
- Dwemer Armors and Weapons Retexture SE
- Ebony Armors and Weapons Retexture SE
- Elven Armors and Weapons Retexture SE
- Falmer Armors and Weapons Retexture SE
- Forsworn Armors and Weapons Retexture SE
- Ghosts of the Tribunal Retexture SE
- Glass Armors and Weapons Retexture SE
- Guards and Stormcloaks Armors Retexture SE
- Imperial Armors and Weapons Retexture SE
- Iron Armors and Weapons Retexture SE
- Leather Armors Retexture SE
- Nightingale Armor and Weapons Retexture SE
- Orcish Armors and Weapons Retexture SE
- Robes Retexture SE
- Silver Armor and Weapons Retexture SE
- Spell Knight Armors Retexture SE
- Stalhrim Armors and Weapons Retexture SE
- Stalhrim Refrozen
- Steel Armors and Weapons Retexture SE
- Thieves Guild Armors Retexture SE
- Unique Armors and Weapons Retexture SE
- Unique Old Gods Armor Survival Mode Keywords
- Vampire Armors and Weapons Retexture SE
- Vigil Enforcer Retexture SE
- Wolf Armors and Weapons Retexture SE
- Enhanced Vanilla Landscapes 
- Skyrim Remastered - Winterhold
- Skyrim Remastered - Castle Volkihar and Clutter
- Skyrim Remastered - Fort Dawnguard
- Bellyaches Animal and Creature Pack Revamp

Updated:
- Light Placer
- Dynamic String Distributor (DSD)
- PGPatcher (aka ParallaxGen)
- Skyland Happy Little Trees Bark
- Lock Bashing
- Stormcloaks Fight With Thalmor
- CS Light
- True Light

Changes:
- DynDOLOD now uses medium mesh rules with some tweaks, so performance should be better
- Fixed snow on trees not matching with the snow on the ground
- Fixed stretched snow textures
- The male version of the Shrouded Cowl now has a mask, like the female version
- Dungeons in the northern regions are now appropriately icy
- Animals should look a lot better, redid visuals for them
- Armours, clothing, weapons, landscapes, and gold/silverware now use PBR 
- Game will now auto-change resolution if your monitor's resolution changes
- Better retextures for various places

2.0.2:
Changes:
- Fixed mod ordering in MO2

2.0.1:

Added:
- SMIM Minewood HD Texture Replacer
- CC's HQ Carts
- Imperial Wood Beams SMIMed
- 4K SMIM Furniture Chest
- 4K SMIM Furniture Improvement
- CC's HQ Roadsigns SSE
- CC's HQ Mines Redone
- SMIM Barrels Retexture
- Skyrim Remastered - Metal Cages and SMIM Imperial Jails
- Improved Jail Bars
- Cave Brazier
- Salmon Retex - Realistic Salmon
- Salmon Retex - Fins Patch
- Diverse Catches - Base Object Swapper Fish Racks
- CC's HQ Caves
- 3D mine doors replacement
- Weathered Stockades Retexture (and its addons)
- Raven Rock Architecture Retexture - 4K - 2K
- Skyland Farmhouses - Towns and Villages
- Improved Mesh Vanilla Handcart Replacement
- chair vanilla style - immersion
- 3D door SE
- 3D wrshackdoor01 SE
- ladder 3d immersion vanilla style
- Vanilla immersion 3D shelves SE
- shackdoor cedr SE
- dragon door full 3D 
- Organic Riften Leaves
- FYX - Riften Canal and Round Posts
- 3D Solitude Market Trellis
- Sconces of Solitude - Unique Solitude Braziers - Base Object Swapper
- slightly Better Bunting (aka those Solitude festival flags)
- Water in Wells - mesh-only animated wells
- Markarth Fixed AF
- FYX - Jorrvaskr
- FYX - RavenRock Docks and Fences Round Posts
- FYX - The Temple of Mara
- FYX - 3D Whiterun Castle Bridge
- FYX - 3D Whiterun Guard Towers
- FYX - 3D Shack Kit Roofs
- FYX - 3D Shack Kit Walls
- FYX - Smooth Wells
- FYX - Windhelm Stable Roof
- FYX - 3D Stockades - Walls and Gate
- FYX - Windhelm Palace of The Kings
- YAR - Yuril's Additional Resources
- FYX - Solitude Arch
- ALT - The Snow Elves Throne
- ALT - Markarth's Forge
- FYX - Black-Briar and Riftweald Manors
- FYX - Eastern Empire Company Building
- RYFTEN - Consistency of windows in Riften
- Shalidor's Maze Sound Fix
- Skyrim Remastered - Castle Volkihar and Clutter
- Skyrim Remastered - High Hrothgar v2
- Skyrim Remastered - Fort Dawnguard
- Praedy's Soul Cairn - SE
- Skeleton Replacer HD - SE 
- Praedy's Chantry of Auriel - SE
- Mrf's Markarth
- Windhelm Fake Windows Fix
- Windhelm Brazier - Vanilla Plus (High-ish Poly) Replacer
- Gravestones 2k-4k
- Skyland Whiterun
- Common Clothing Retexture SE
- Robes Retexture SE
- Dragon Armors and Weapons Retexture SE
- Ebony Armors and Weapons Retexture SE
- Daedric Armors and Weapons Retexture SE
- LeanWolf's Better-Shaped Weapons SE
- Orcish Armors and Weapons Retexture SE
- Iron Armors and Weapons Retexture SE
- Steel Armors and Weapons Retexture SE
- Imperial Armors and Weapons Retexture SE
- Elven Armors and Weapons Retexture SE
- Glass Armors and Weapons Retexture SE
- Wolf Armors and Weapons Retexture SE
- Silver Armor and Weapons Retexture SE
- Leather Armors Retexture SE
- Falmer Armors and Weapons Retexture SE
- Ancient Nord Armors and Weapons Retexture SE
- Nightingale Armor and Weapons Retexture SE
- Dawnguard Armors and Weapons Retexture SE
- Dwemer Armors and Weapons Retexture SE
- Thieves Guild Armors Retexture SE
- Forsworn Armors and Weapons Retexture SE
- Dragon Priests Retexture SE
- Unique Armors and Weapons Retexture SE
- Spell Knight Armors Retexture SE
- Ghosts of the Tribunal Retexture SE
- Vigil Enforcer Retexture SE
- Divine Crusader Retexture SE
- Saints and Seducers Retexture SE
- Dark Brotherhood Armors Retexture SE
- Chitin Armors Retexture SE
- Stalhrim Armors and Weapons Retexture SE
- Better Dirt Cliffs and Alphas
- Remove All Ugly Dead Shrubs and Thickets
- ERM - Enhanced Rocks and Mountains - DynDOLOD Add-On
- Enhanced Rocks and Mountains - Complex Material and PBR
- Bellyaches Animal and Creature Pack Revamp
- Iconic's Dragon Retexture
- Iconic's Hagraven Retexture
- Iconic's Werewolf and Werebear Retexture
- Iconic's Vampire Lord Retexture
- Iconic's Spiders of Skyrim Retexture
- Iconic's Gargoyle and Death Hound Retexture
- Natura - Stone Atronach Retexture
- Iconic's Ashman Retexture
- Iconic's Frost - Flame - Storm Atronach Retexture
- Iconic's Ash Guardian Retexture
- Iconic's Spriggan Retexture
- Feral - Claw Unarmed Attacks for Beast Races - Vampires - Werewolves
- Survival Mode Improved - SKSE (disabled by default)
- Papyrus MessageBox - SKSE NG
- SkyPatcher - NARC Remade - No Animals Report Crimes
- Player Panting Loop Fix
- Imperial Thalmor Stormcloak Patrols Fixes - Soldiers and Prisoners World Encounters
- The Clever Prisoner
- Equip Enchantment Fix
- Torch Mechanics Fixed
- Safer Spikes
- Kanjs - Torch
- Lexicon SKSE
- Actor Value Generator
- Dynamic String Distributor
- Navigator - Navmesh Fixes
- Civil War Champions - Proper Male Weighting
- Unique Red Wave
- Functional Adventurer's Backpack - Innate Carry Weight Boost
- Betty Netch
- HD Terrain Noise Texture SE
- Beast Race Vampire Fang Removal
- Dragonsreach Designs - Carvings Retexture

Extended Profile:
- Skyrim Vanilla Plus Overhaul
- Perk Trees - Unlocked Mechanics and Tweaks
- Blade and Blunt
- Rare Curios - Bolts Expanded
- Thwack - Crossbow Collection Reworked
- Kachunk - Creation Club Crossbow Distribution
- Staff Enchanting Plus
- Alchemy Plus
- NPCs Ain't Hachikos
- AI Overhaul SSE
- Constellations - Additional Player Skills
- Firmanent
- On a Crimson Trail
- Ghosts of the Tribunal - Reduced Cut
- The Cause - Reduced Cut
- Fishing - Reduced Cut
- Commanding Authority - Civil War Champions Reintegrated
- Morihaus' Refuge - Lord's Mail Reintegration
- Menagerie - An Anniversary Edition Pet Overhaul
- NPCs Take Cover - Smarter Anti-Cheese AI
- Animals Swim (Sort of)
- Locked Chests Have Keys
- Lock Bashing
- Better Carriage Destinations
- Wait Carriage in Inns - Fast Travel Improvement
- Housecarls Pre-Thaneship
- Simple Winterhold Housecarl
- Alternate Forms Keep Utility
- Call of Valor Tweaks
- CAM - Companions at Mirmulnir
- Andrealletius' Scene Tweaks
- Additional Dremora Faces
- Erandur Dialogue Edits
- Hunters Sell Fish and More Animal Parts
- Lore Gameplay Tweaks
- No Swing Warnings While Wielding Tools
- Whiterun Watchtower Doesn't Start Broken
- Environs - Whiterun Watchtower Doesn't Stay Broken
- Environs - Abandoned Abodes
- Whiterun Watchtower Doesn't Start Broken
- Environs - Hroggar's House
- Environs - Riften Warehouse
- Environs - Master Plugin
- Environs - The Shrines of Talos
- Environs - Kolskeggr
- Environs - The Greenwood Shack
- Environs - The Ruined Tundra Farmhouse
- NPC Mining Yields Ore
- Say No To Calcelmo
- No Friendship for Rolff
- Neutral Whiterun Guards
- Stormcloaks Fight With Thalmor
- Thieves Guild Holdup (Thief Road Encounter) Improvements
- Thieves Guild Master Recognition
- DEEJMASTER's Mini Mods Tweaks and Fixes
- Vanilla Purity Patch
- Canis Hysteria - The Werewolf Disease
- Scion - A Vampire Overhaul
- Manbeast - A Werewolf Overhaul
- Vampire Feeding Tweaks
- Vampire Lords Can Fly (With Collision)
- The Companions - Don't be a Milk Drinker
- More Werewolves SE
- Forsworn Skinchangers
- Cannibal Draugr on Solstheim (Bloodmoon Creature Restoration Project)
- More Hircinic Aspects of Hircine- Mihail's Shards of Immersion (SE-AE version)
- Udefrykte. Uderfrykte. Udyrfrykte. (Bloodmoon Creature Restoration Project)
- The Blood Horker (Bloodmoon Creature Restoration Project) SE
- Karstaag - The Frost King Reborn (Bloodmoon Creature Restoration Project) SE
- GRAHL - The Ice Troll (Bloodmoon Creature Restoration Project) SE
- Stendarr's Chosen - Stendarr's Hammer Reintegration
- More Morrowind-y Staff Enchanter
- Mainland Staff Enchanters
- Fire Hurts NG Updated
- SkyParkour v3 - Procedural Parkour and Climbing Framework (SPPF)
- Locational Damage AE (SKSE Plugin)
- 

Removed:
- Cathedral - 3D Thicket and Dead Shrub
- Optional Quick Start (for Extended Profile, still in the base profile)

Updated:
- Enhanced Vanilla Landscapes
- Ambient Templates for Lighting Mods
- Synthesis
- SkyPatcher

Changes:
- ESL'ified Quick Start Shrub Fix
- Adjusted lighting values in Ambient Templates for Lighting Mods, so that dungeons are actually dark
- Patched a record conflict between Skyrim Children Refine and Misc Bugfix Compilation
- New extended profile with Skyrim Vanilla Plus Overhaul, Perk Trees - Unlocked Mechanics and Tweaks, Blade and Blunt (installed with vanilla difficulty modifiers)
